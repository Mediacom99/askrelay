...
// pins that residue. The delivery layer must therefore match Thread() AND
// ID() against the message it is about to transmit, never just check non-nil
// (recorded as a WP-08 requirement in the plan).
func TestZeroReleaseIsInert(t *testing.T) {
	rel := new(gate.Release)
	if rel.Thread() != "" || rel.ID() != "" || rel.Payload() != nil || rel.ViaGrant() {
		t.Errorf("zero Release leaks data: thread=%q id=%q payload=%v viaGrant=%v",
			rel.Thread(), rel.ID(), rel.Payload(), rel.ViaGrant())
	}
}

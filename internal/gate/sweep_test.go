...
func TestOutboundSweep(t *testing.T) {
	a := Approvable{Kind: KindMessage, Direction: Outbound, Thread: "01ABC", Payload: "reply"}
	for _, s := range sweepDraftStates {
		for _, v := range sweepVerdicts {
			got, rel, err := ApplyOutbound(a, s, v)
			legal := s == PendingReview && (v == Approve || v == Reject)
			if wantRel := legal && v == Approve; (rel != nil) != wantRel {
				t.Errorf("(%q, %q): release present = %v, want %v", s, v, rel != nil, wantRel)
			}
			if rel != nil && rel.ViaGrant() {
				t.Errorf("(%q, %q): manual release marked via_grant", s, v)
			}
			if legal {
				want := Sent
				if v == Reject {
...
func TestGrantSweepInbound(t *testing.T) {
	a := Approvable{Kind: KindMessage, Direction: Inbound, Thread: "01ABC", Payload: "question"}
		...
		for _, s := range sweepDraftStates {
			...
			if !g.Covers(a) {
				if !errors.Is(err, ErrNoGrant) || got != "" || rel != nil {
					t.Errorf("grant %+v uncovered: got (%q, %v, %v), want (\"\", nil, ErrNoGrant)", g, got, rel, err)
				}
				continue
			}
			wantSt, wantRel, wantErr := ApplyOutbound(a, s, Approve)
			if got != wantSt || !errors.Is(err, wantErr) || (rel != nil) != (wantRel != nil) {
				t.Errorf("grant %+v state %q: got (%q, rel=%v, %v), want (%q, rel=%v, %v)",
					g, s, got, rel != nil, err, wantSt, wantRel != nil, wantErr)
			}
			if rel != nil && !rel.ViaGrant() {
